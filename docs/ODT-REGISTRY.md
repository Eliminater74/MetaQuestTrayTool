...
- **High bitrate values:** v1.1.33 adds presets through 960 Mbps. v1.1.34 startup preservation covers both `BitrateMbps` and `DBRMax`: if ODT already has either value above the old 500 Mbps preset ceiling and the saved tray baseline is still old-capped, the tray preserves the higher ODT value before writing the rest of the Link baseline. v1.1.35 skips the startup Link write entirely if that preflight read fails.
