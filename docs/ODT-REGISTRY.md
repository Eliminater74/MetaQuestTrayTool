...
- **High bitrate values:** v1.1.33 adds presets through 960 Mbps. On startup, if ODT already has a bitrate above the old 500 Mbps preset ceiling and the saved tray baseline is still old-capped, the tray preserves the higher ODT value before writing the rest of the Link baseline.
